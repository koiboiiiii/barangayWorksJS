  ...
  if (calendarRoot) {
    ...
    const saveScheduleDate = async (scheduleDate, isUnavailable) => {
      try {
        ...
        // Determine success: prefer HTTP 2xx but also accept JSON { ok: true }
        let body = null;
        try { body = await resp.clone().json(); } catch (e) { /* ignore non-json */ }
        const success = (resp && resp.ok) || (body && body.ok);
        if (!success) {
          // log details for debugging but keep the pending override
          try { console.warn('[saveScheduleDate] non-ok response', resp && resp.status, body); } catch (e) {}
          return false;
        }
        ...
        try { console.warn('[saveScheduleDate] fetch error', error && error.message); } catch (e) {}
        return false;
      }
    };
