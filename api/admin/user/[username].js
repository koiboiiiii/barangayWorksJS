...
module.exports = async (req, res) => {
  try {
    ...
    const target = new URL(`/api/admin/user/${encodeURIComponent(username)}`, backendBase).toString();
    ...
    const fetchRes = await fetch(target, {
      ...
      body: req.method === 'PATCH' || req.method === 'POST' || req.method === 'PUT' || req.method === 'DELETE'
        ? await (async () => { const chunks = []; for await (const c of req) chunks.push(c); return Buffer.concat(chunks); })()
        : undefined
    });
